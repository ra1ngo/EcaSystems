...
StateRegistry.Register<T>(Func<IEcaRuleState,T>) регистрирует способ получить actual external state; State.Resolve<T>(ruleState) передаёт текущий state явно, без Bind. Данные принадлежат внешней System. Base RuleState содержит RuleId; внешняя System сама выбирает global/per-Rule/per-Scope state или координату ScopeId + RuleId.
