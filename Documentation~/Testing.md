...
Добавлены 22 NUnit cases: EcaEventDispatcherTests — 8, EcaEventReceiverTests — 8, EcaEventsBaseRuntimeTests — 6. Проверены occurrence identity/state, declared E для DerivedState/null и value types, независимые outbound/inbound registry checks, ложный EventStateType, source contract, single-handler subscription/unsubscribe/Dispose, propagation ошибок, arbitrary multi-E через один Receiver subscription, Conditions/barrier и reentrant dispatch. Test-only EcaTestBaseRuntime адаптирует Handle<E> → Fire<E> → Base Fire с generic State<E>/contexts; production EventHandling не зависит от runtime layers. Существующие Core2 tests обновлены только для имени Fire, semantics обоих runtime overloads сохранена.

Три локальных Unity 6000.5.6f1 batchmode EditMode запуска: новые Events — **22 passed**, весь Core2 — **172 passed**, полный suite — **286 passed** (Core — 37, Core1 — 77, Core2 — 172). Во всех запусках **0 failed, 0 skipped, 0 inconclusive**, exit code 0. XML/log находятся в игнорируемой `.validation~`: `core2-events-focused-results.xml` / `core2-events-focused.log`, `core2-events-core2-results.xml` / `core2-events-core2.log`, `core2-events-full-results.xml` / `core2-events-full.log`. PlayMode scenarios отсутствуют; удалённый GameCI не запускался.

## PR #12 EventHandling follow-up — 2026-09-15

Добавлены 3 NUnit cases для public occurrence factory/custom source: declared BaseState с DerivedState/null, один subscription для class/struct, validation того же Event instance, который получает handler. Адаптированы 9 существующих cases к internal Accept и factory вместо test subclass. Все tests используют public API без friend access/reflection; internal constructor и internal abstract Accept закрывают внешнее наследование на уровне C# contract. Папка Concept переименована в EventHandling; все 6 прежних `.meta` перенесены без изменения содержимого/GUID.

Unity 6000.5.6f1, три batchmode EditMode запуска: EventHandling — **25 passed**, все Core2 — **175 passed**, полный suite — **289 passed** (Core — 37, Core1 — 77, Core2 — 175). Везде **0 failed, 0 skipped, 0 inconclusive**, exit code 0. XML/log сохранены вне commit в `.validation~`: `pr12-eventhandling-focused-results.xml` / `pr12-eventhandling-focused.log`, `pr12-eventhandling-core2-results.xml` / `pr12-eventhandling-core2.log`, `pr12-eventhandling-full-results.xml` / `pr12-eventhandling-full.log`. PlayMode и удалённый GameCI не запускались. Base/Execution/Scope/Commands/Core/Core1 в follow-up не изменены.
