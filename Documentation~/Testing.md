...
Добавлены 22 NUnit cases: EcaEventDispatcherTests — 8, EcaEventReceiverTests — 8, EcaEventsBaseRuntimeTests — 6. Проверены occurrence identity/state, declared E для DerivedState/null и value types, независимые outbound/inbound registry checks, ложный EventStateType, source contract, single-handler subscription/unsubscribe/Dispose, propagation ошибок, arbitrary multi-E через один Receiver subscription, Conditions/barrier и reentrant dispatch. Test-only EcaTestBaseRuntime адаптирует Handle<E> → Fire<E> → Base Fire с generic State<E>/contexts; production Events не зависит от runtime layers. Существующие Core2 tests обновлены только для имени Fire, semantics обоих runtime overloads сохранена.
