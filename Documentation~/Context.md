...
`Runtime/Systems/Time/Core` — самостоятельная Unity time/timer system, assembly и namespace `EcaSystems.Time`. Нет зависимостей от Core/Core1/Core2 или ECA concepts. Система и изолированные tests рассчитаны на дальнейший перенос в отдельный repository/package. `Runtime/Systems/Time/Eca` зарезервирован только папкой и `.meta`; ECA adapter, descriptor, Events/Commands/State/Signals пока отсутствуют.

`TimeSystem` — обычный public sealed instance class с public constructor без параметров, не singleton/MonoBehaviour. Приложение само хранит его reference. API: `CreateTimer(TimerCreateOptions)`, `Get(string)`, `TryGet(string, out Timer)`, `Start/Stop/Pause/Resume(string)`, `DestroyTimer(string)`, `Awaitable Wait(double, TimerScaleMode = Scaled)`. Все операции и callbacks выполняются на Unity main thread. Public Dispose/Attach/Detach, cancellation и отдельного Timer mutation API нет.

`TimerCreateOptions(id, duration, scaleMode = Scaled)` — immutable DTO. Creator проверяет options/ID, finite non-negative duration и enum. ID уникален внутри instance с Ordinal semantics. Null/empty/whitespace ID даёт ArgumentException, null options — ArgumentNullException, неправильная duration/scale — ArgumentOutOfRangeException. Duplicate/unknown ID и invalid lifecycle transition — InvalidOperationException; TryGet неизвестного корректного ID возвращает false.

`Timer` — public state/event object: get-only Id, Duration, Elapsed, Remaining, Progress, ScaleMode, State; C# events `Started`, `Stopped`, `Paused`, `Resumed`, `Completed`, `Destroyed` передают exact Timer. Lifecycle mutation internal и доступна пользователю через TimeSystem по string ID. `TimerState`: Stopped/Running/Paused/Completed/Destroyed. Start из Stopped/Completed начинает полный запуск; Pause только из Running, Resume только из Paused без Started; Stop из Running/Paused/Completed сбрасывает Elapsed/Progress в 0 и Remaining в Duration. Invalid transitions не вызывают events.

Natural completion оставляет Timer в registry с Elapsed = Duration, Remaining = 0, Progress = 1. Destroy сначала удаляет регистрацию, затем устанавливает terminal Destroyed и вызывает только Destroyed; metadata и последние timing values читаются дальше. ID сразу доступен для нового exact instance, в том числе из Destroyed callback. Duration 0: Started синхронно, Completed только на следующем tick; Progress до completion равен 0, после — 1, включая сохранённую destroyed reference.

Внутри Creator отвечает за validation/create/register, Registry — только Dictionary/lookup/removal, Runner — scan активных таймеров и completion. TimeSystem делегирует orchestration этим объектам. Timing использует double и absolute end timestamp: Scaled — Unity `Time.timeAsDouble`, Unscaled — `Time.unscaledTimeAsDouble`. Elapsed/Remaining/Progress читаются по текущему clock, clamp до границ; Pause замораживает значения, Resume переносит deadline на сохранённый остаток. Snapshot runner хранит также версию запуска: restart другого Timer из callback не завершает новый запуск по старому snapshot.

Internal static PlayerLoop hub добавляет один delegate в конец обычной Update phase, сохраняя остальные nodes. Перед установкой удаляются только собственные hooks, поэтому повторная initialization не создаёт дубликатов. SubsystemRegistration очищает active work предыдущей play session; BeforeSceneLoad устанавливает hook, первая active работа также проверяет установку. Между Scene loads hub не сбрасывается. Нет GameObject, DontDestroyOnLoad или scene ownership.

Hub удерживает только runners с Running timers/Wait; последний Pause/Stop/Destroy/completion снимает runner. Idle instances/registries глобально не хранятся. Уже запущенная работа продолжается даже после потери caller reference; paused scaled time может удерживать её до возобновления clock. Это lifetime активной работы, а не public singleton или Dispose contract. Instances предназначены для одной play session, не для переноса через выход/повторный вход в Play Mode без domain reload. Нового public lifecycle API для этой модели не требуется.

State transition и active membership фиксируются до lifecycle callback. Обычные C# event exceptions распространяются caller'у; при completion Runner сначала обрабатывает остальные выбранные timers, затем возвращает AggregateException, а PlayerLoop hub логирует её и продолжает другие runners. Exceptions не откатывают уже совершённый transition. Reentrant callbacks могут остановить, уничтожить или снова запустить Timer.

Wait создаёт приватный Timer с теми же progression/Runner и AwaitableCompletionSource. Этот Timer не регистрируется, не имеет пользовательского ID и не виден Get/TryGet, поэтому нет generated-ID collisions. Completion удаляет active work до SetResult; continuation может сразу создать новый Wait. Public результат — Unity Awaitable, не Task; каждый Awaitable следует ожидать только один раз. Будущий ECA adapter сам адаптирует его к Command contract. Cancellation, repeat, custom scale и другие возможности v1 не реализованы.
...
`AEcaCommand` — public non-generic runtime abstract class: Id, ContextType, ArgsType и public `Task Run(IEcaActionContext, object)`. `AEcaCommand<C,A>` — typed authoring abstract class с `C : IEcaActionContext`, abstract `Run(C,A)` и sealed overrides metadata/erased bridge. Class hierarchy выбрана намеренно из-за Unity/Mono/IL2CPP compatibility concerns generic default/explicit interface implementations; используется обычный virtual dispatch. Generic variance исчезла. Concrete Command реализует Id и typed Run, без metadata/casts. Entry, reflection/dynamic и compatibility aliases отсутствуют.

EcaCommandRegistry хранит AEcaCommand напрямую: `Register(AEcaCommand)`, `Unregister(string)`, `Contains(string)`, internal `Resolve(string)`. Heterogeneous AEcaCommand[] регистрируется без знания C/A. Runner сохраняет проверки context, declared args type, nullable/null и null Task; runtime bridge остаётся public, validation выполняется Runner до вызова.

IEcaEventRegistry/EcaBaseEventRegistry получили `Register(IEcaEvent)`, `Unregister(string)` и `Contains(string)`. Generic Register<E> сохранён с проверкой EventStateType == typeof(E); non-generic composition использует предоставленную декларацию без восстановления E. CheckRegistered по-прежнему проверяет Id + metadata type. Contains/Unregister отклоняют null/empty/whitespace ID через ArgumentException. Duplicate registration — InvalidOperationException; comparer Ordinal.

`Runtime/Core2/Systems` содержит пять public sealed типов в плоском namespace EcaSystems.Core2: EcaSystem, EcaSystemNamespace, EcaSystemRegistry, EcaSystemNamespaceRegistry, EcaSystemConnector. System — passive descriptor/export provider, не центральный runtime. Constructor получает id, systemNamespace, optional IReadOnlyCollection<IEcaEvent> events и IReadOnlyCollection<AEcaCommand> commands, optional name/description. Namespace содержит только Id; Name/Description System не валидируются. Null collections заменяются Array.Empty, полностью пустой System разрешён.

System хранит прямые references на Namespace и переданные collections без defensive copy/wrapper/snapshot. Конфигурация и metadata должны оставаться неизменными runtime. Если mutation реально появится, сначала выясняется её источник, затем проектируется защита. Один attached System соответствует одному уникальному Namespace. EcaSystem.Id временно сохранён, хотя частично дублирует Namespace.Id; их роли будут пересмотрены вместе с развитием Namespace.

ID задаются caller'ом целиком, без automatic namespace prefixing. Уникальность per type/per registry: Event и Command могут иметь одинаковую строку Id; System/Namespace также не конфликтуют с другими типами. Обратных ссылок Event/Command на System нет. IEcaEventEmitter не является property System и будет передаваться adapters отдельной будущей композицией.

EcaSystemRegistry предоставляет Register/Unregister/Contains и internal Resolve для exact-reference проверки. EcaSystemNamespaceRegistry предоставляет Register/Unregister/Contains. Это простые single-collection stores; cross-registry orchestration принадлежит только EcaSystemConnector(systems, namespaces, events, commands), все зависимости обязательны.

Attach(EcaSystem) полностью проверяет System/Namespace ID, наличие Namespace, null exports, валидность/внутренние дубликаты export IDs и отсутствие конфликтов в соответствующих registries до первой mutation. Затем регистрирует Namespace → Events → Commands → System. Detach(EcaSystem) проверяет exact зарегистрированный System instance и наличие Namespace/exports по ID, затем удаляет System → Commands → Events → Namespace. Повторный Attach/Detach и иной instance того же System.Id дают exception.

Rollback обоих методов — локальный стек обратных действий только для успешно выполненных шагов. Attach удаляет добавленные регистрации в обратном порядке; Detach заново регистрирует удалённые исходные references в обратном порядке. После успешной компенсации исходная ошибка пробрасывается без замены; при ошибке компенсации остальные undo всё равно выполняются, AggregateException сообщает исходную и rollback errors. Новые transaction/ownership/refcount abstractions не потребовались. Гарантия возврата к прежнему состоянию относится к согласованным exception-safe registry operations без внешней/concurrent mutation; произвольный custom IEcaEventRegistry, изменяющий состояние и затем бросающий exception или отказывающий в компенсации, не может получить универсальную atomicity от этого API. Такие ошибки не скрываются.

Systems v1 не содержит State/SystemState, Queries, TimeSystem/WaitCommand, EcaSystemsRuntime, enable/disable, serialization, routing или adapter binding. Новые Systems tests в этой итерации намеренно не добавляются; существующие tests адаптированы и запускаются.
