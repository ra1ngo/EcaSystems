...
## Core2 Events concept — 2026-09-15

`Runtime/Core2/Concepts/Events` — самостоятельный horizontal Concept с зависимостями только от System и Base event contracts/registry. Поток: `Dispatcher.Fire<E> → EcaEventOccurrence<E> → IEcaEventOccurrenceSource.Fired → Receiver → CheckRegistered → occurrence.Accept(handler) → IEcaEventHandler.Handle<E> → Runtime.Fire<E>`. Dispatcher и Receiver хранят EcaBaseEventRegistry и отдельно проверяют регистрацию на outbound/inbound границах; Dispatcher также сверяет EventStateType с typeof(E). Identity остаётся принятой в Base: Id + EventStateType, metadata после регистрации должна быть стабильной.
...
`IEcaEventOccurrenceSource` и абстрактный `EcaEventOccurrence` публичны, чтобы Receiver мог принимать независимые источники из других assemblies. Base occurrence имеет Event, protected constructor и Accept; generic `EcaEventOccurrence<E>` — internal sealed transport implementation. Единственный C# event явно реализован в Dispatcher через source interface; private delegate служит backing storage. Receiver не публикует второй event. Occurrence сохраняет исходный declared E, включая BaseState с DerivedState instance, value types и null, без dynamic/reflection и без object на границе callback.
