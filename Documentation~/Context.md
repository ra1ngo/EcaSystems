...
TimeEcaEvents удалён. Adapter содержит public constants ECA_EVENT_TIMER_STARTED_ID / STOPPED / PAUSED / RESUMED / COMPLETED / DESTROYED с прежними time.timer.* строками. Setup регистрирует declarations по этим constants; adapter один раз Resolve-ит и кеширует IEcaEvent<EcaTimeEventState>, проверяя interface и EventStateType. Несовместимость обнаруживается в constructor. EcaTimeEventState (бывший TimerEventState) — immutable snapshot TimerId/Duration/Elapsed/Remaining/Progress/ScaleMode/State, создаваемый до Fire; reentrant handling не меняет отправленный payload.

TimeEcaCommandIds и generic command wrapper удалены. Отдельные public EcaCreateTimerCommand, EcaStartTimerCommand, EcaStopTimerCommand, EcaPauseTimerCommand, EcaResumeTimerCommand, EcaDestroyTimerCommand, EcaWaitCommand владеют собственными Id/ID и обязательной TimeSystem dependency. Типы args прежние: TimerCreateOptions, string ID, TimeWaitArgs. Контекст — IEcaActionContext, business result отсутствует. Только EcaWaitCommand делает Awaitable → Task bridge.

Production Core2 composition, общий lifecycle/ownership adapters и Rule creation по Event ID остаются следующей стадией. В PR #17 follow-up изменены только согласованные simple registries и registry-backed EcaSystem; EventEmitter.Fire<E>(IEcaEvent<E>, E) не менялся, ID-based Fire отсутствует. State/Queries, Signals, routing, generic Registry/adapter abstraction и deferred Time features не реализованы.
