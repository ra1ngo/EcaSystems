...
Public API: `IEcaEventEmitter.Fire<E>(IEcaEvent<E> ecaEvent, E eventState)`; `EcaEventEmitter(IEcaEventRegistry events)` реализует этот порт; `IEcaEventHandler.Handle<E>(IEcaEvent<E> ecaEvent, E eventState)` — runtime-side generic callback. Emitter проверяет null Event, CheckRegistered и соответствие EventStateType объявленному typeof(E), затем передаёт тот же Event/State одному handler. Registry остаётся живой dependency; metadata после регистрации должна быть стабильной.
