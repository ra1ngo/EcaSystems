...
## Core2 EventHandling concept — 2026-09-15

`Runtime/Core2/Concepts/EventHandling` — самостоятельный horizontal Concept с зависимостями только от System и Base event contracts/registry. Поток: `Dispatcher.Fire<E> → EcaEventOccurrence<E> → IEcaEventOccurrenceSource.Fired → Receiver → CheckRegistered → occurrence.Accept(handler) → IEcaEventHandler.Handle<E> → Runtime.Fire<E>`. Dispatcher и Receiver хранят EcaBaseEventRegistry и отдельно проверяют регистрацию на outbound/inbound границах; Dispatcher также сверяет EventStateType с typeof(E). Identity остаётся принятой в Base: Id + EventStateType, metadata после регистрации должна быть стабильной.

API: `EcaEventDispatcher(EcaBaseEventRegistry events).Fire<E>(IEcaEvent<E>, E)`; `EcaEventReceiver(EcaBaseEventRegistry events, IEcaEventOccurrenceSource source)` с `Subscribe(IEcaEventHandler)`, `bool Unsubscribe(IEcaEventHandler)`, `Dispose()`; `IEcaEventHandler.Handle<E>(IEcaEvent<E>, E)`. Receiver хранит одного consumer, не знает concrete Dispatcher/runtime и вызывает Accept напрямую. Повторный Subscribe при активном handler бросает InvalidOperationException; Unsubscribe сравнивает instance, после снятия можно подписать новый. Dispose один раз отсоединяет source, очищает handler и закрывает Subscribe/Unsubscribe через ObjectDisposedException. Незарегистрированный Event даёт InvalidOperationException даже без handler; null arguments — ArgumentNullException, несовместимый generic metadata в Dispatcher — ArgumentException. Ошибки callback распространяются синхронно.

Base определяет модель Event (IEcaEvent/IEcaEvent<E>/EventRegistry), а EventHandling — обработку occurrence. `IEcaEventOccurrenceSource` и абстрактный `EcaEventOccurrence` публичны для независимых источников из других assemblies. Public API occurrence: read-only `Event` и `static EcaEventOccurrence Create<E>(IEcaEvent<E> ecaEvent, E eventState)`. Constructor и Accept — internal: внешний source не может наследовать occurrence и подменить Event между validation и Handle<E>. Generic `EcaEventOccurrence<E>` остаётся internal sealed framework implementation и передаёт handler тот же Event instance, который проверяет Receiver.

Custom source публикует `Fired?.Invoke(EcaEventOccurrence.Create(ecaEvent, eventState))`; Dispatcher использует ту же factory. Factory не требует registry, отклоняет null Event; входную регистрацию по-прежнему проверяет Receiver. Единственный C# event явно реализован в Dispatcher через source interface; private delegate служит backing storage. Receiver не публикует второй event. Occurrence сохраняет исходный declared E, включая BaseState с DerivedState instance, value types и null, без dynamic/reflection и без object на границе callback. Namespace остаётся плоским `EcaSystems.Core2`; при rename папки в EventHandling сохранены все `.meta`/GUID.

Один Subscribe обрабатывает произвольные пользовательские E без per-event subscriptions. Это проверено test-only `EcaTestBaseRuntime : EcaBaseRuntime, IEcaEventHandler`: Receiver передаётся dependency и один раз подписывает runtime; Handle<E> вызывает простой Fire<E>, который адаптирует state/contexts к generic Base Fire. Production Base не знает о Receiver. Rule selection и ALL CONDITIONS → ALL ACTIONS остаются в Base, dispatch синхронный/reentrant.
