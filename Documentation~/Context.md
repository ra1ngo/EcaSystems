...
Каждый EcaScope создаёт собственный готовый IEcaEventEmitter EventEmitter и bind'ит его к this. ScopeRuntime передаёт общий live EventRegistry, но не владеет emitter. Сохранённый emitter удерживает конкретный Scope instance; после Dispose получает ObjectDisposedException, включая вызовы с уже удалённым Event или null, и никогда не перенаправляется в replacement с тем же ScopeId. Внутренняя проверка доступности Scope до event validation сохраняет приоритет disposed failure прямого Fire. Handler ведёт в тот же Scope.Fire; public Handle нет. Dispose не отменяет уже запущенные Actions.
...
Следующая отдельная production-composition итерация — EcaSystemsRuntime: владеет EcaScopeRuntime (не ExecutionGroupRegistry), global System/Event/Command/Namespace registries, EcaSystemConnector и EcaCommandRunner; root Scope автоматически не создаётся. Lifecycle adapters остаётся вне Core/Runtime. CreateRule, simplified/typed Action/Condition helpers с учётом visual programming, context enrichment/capability container и cross-scope routing отложены. После Attach local exports должны быть стабильны по convention; freeze/snapshot/ownership/consistency — Roadmap.
...
TimeEcaAdapter(TimeSystem, local IEcaEventRegistry, IEcaEventEmitter) — только live bridge. TimeEcaSetup.CreateSystem(time) создаёт passive EcaSystem с local Event/Command registries, а caller передаёт system.Events в adapter. Namespace/System Id = time. Connect подписывает шесть aggregate events; повторный Connect даёт InvalidOperationException, Disconnect допускает повторный вызов. Порядок: Connector.Attach(system) → adapter.Connect(); adapter.Disconnect() → Connector.Detach(system). Ошибка Fire до Attach не откатывает standalone Timer transition.
...
Единственный public порт Concept: `IEcaEventEmitter.Fire<E>(IEcaEvent<E> ecaEvent, E eventState, IEcaConditionContext conditionContext = null, IEcaActionContext actionContext = null)`. `EcaEventEmitter` — internal sealed implementation с конструктором от IEcaEventRegistry; `IEcaEventHandler.Handle<E>(IEcaEvent<E> ecaEvent, E eventState, IEcaConditionContext conditionContext, IEcaActionContext actionContext)` — internal runtime-side generic callback. Concrete emitter принадлежит конкретному EcaScope, который явно реализует internal handler; внешние Systems/adapters получают только готовый IEcaEventEmitter. Emitter проверяет null Event, CheckRegistered и соответствие EventStateType объявленному typeof(E), затем передаёт тот же Event/State и оба nullable contexts одному handler. Registry остаётся живой dependency; metadata после регистрации должна быть стабильной.
...
Test-only `EcaTestBaseRuntime : EcaBaseRuntime, IEcaEventHandler` один раз bind'ится к Emitter, а Handle<E> вызывает простой Fire<E> и создаёт test RuleState и передаёт полученные nullable contexts в Base Fire<E,R>. Production Base не знает об Emitter; Rule selection и ALL CONDITIONS → ALL ACTIONS остаются в Base. Вызовы синхронные/reentrant. Systems exports реализованы отдельно; Signals, State, production Commands context composition, FireEventCommand, cross-scope routing и Unity bridge здесь не реализованы. Следующая отдельная итерация external adapters описана в Roadmap; сейчас доступен только прямой вызов Fire<E>.
...
EcaCommandRegistry хранит AEcaCommand напрямую и предоставляет Commands (read-only live view), Register, Unregister, Contains, public Resolve и CheckRegistered. Heterogeneous Commands регистрируются без C/A на call site. Runner сохраняет проверки context/declared args/null/null Task.

IEcaEventRegistry/EcaBaseEventRegistry предоставляют Events (read-only live dictionary values), Register(IEcaEvent), Unregister/Contains/Resolve(string), CheckRegistered(IEcaEvent). Generic Register<E> удалён: registry хранит non-generic declarations, typed validation принадлежит потребителю (Emitter, Rule registration, Time adapter). Resolve unknown ID бросает InvalidOperationException; null/empty/whitespace ID — ArgumentException.
...
System хранит local registries без копирования. Их contents и metadata должны оставаться стабильными, пока System attached; синхронизация дальнейших local mutations с global registries автоматически не выполняется. Один attached System соответствует уникальному Namespace; Namespace содержит только Id, automatic prefixing отсутствует.
...
Attach проверяет conflicts до mutation и регистрирует exact instances из system.Events.Events и system.Commands.Commands: Namespace → Events → Commands → System. Detach проверяет canonical identity System, Namespace и всех exports до удаления: System → Commands → Events → Namespace. Foreign instance с тем же ID не удаляется. Local registries сохраняются; Connector не владеет adapter subscriptions.
...
Systems layer не содержит State/SystemState, Queries, EcaSystemsRuntime, enable/disable, serialization, routing или adapter binding. Отдельный Time/Eca уже реализован. PR #17 follow-up добавляет focused registry/Connector tests, включая Attach/Detach rollback и canonical identity.

## Core2 Commands validation — 2026-09-15

`Core2/Concepts/Commands` проверен как самостоятельный horizontal Concept: standalone registry/runner/binding и end-to-end `Commands + EcaBaseRuntime`. В validation-итерации production API не менялся; последующий refactor 2026-09-17 описан выше. Commands подключаются тестовым composition context через `IEcaCommandsActionContext.Commands`; Base получает обычный IEcaActionContext и не знает о Commands. Production integration с Execution/Scope contexts пока не реализована.

`Bind` удерживает ровно переданный ActionContext instance; bindings одного runner/registry/command не смешиваются, в том числе при overlapping async calls. Lookup выполняется заново на каждом Run, поэтому existing binding видит unregister/re-registration. Get/TryGet не добавлены; теперь public Resolve, Commands и CheckRegistered предоставляют inspection. Базовое функциональное parity со старым Core сохранено: registration, lookup, binding, typed invocation, несколько Commands и Task.
...
Fire с createState — локальный pass-through в Execution/Base с Base constraints: сохраняет Base barrier, не использует Scope extendState и ExecutionMode/lifecycle/counters. Caller сам передаёт createState. StateBuilder/StateFactory отложены до реальной необходимости. EventEmitter concept подключён к конкретному Scope. FireEventCommand, production Commands contexts, cross-scope Fire, Unity bridge, cancellation и Reset/Queue не реализованы. Roadmap о внешней мутации registries/Rule.Id сохранён без новой защиты в Execution.
...
Ошибка Condition или её createState может прервать Fire до запуска executions; дополнительной error-policy нет. Cancellation, Reset/Queue, production Commands context composition, Fire Event command и Unity bridge в Core2 Execution не реализованы. EventEmitter принадлежит Scope и вызывает существующий Execution pipeline.
