...
## Core2 Scope/System lifecycle — 2026-09-26

Актуальная production-архитектура — Runtime/Core2. Scope-layer зависит только от собственного EcaScopeLifecycle / IEcaScopeLifecycleListener, не от Systems-layer. EcaScopeRuntime получает optional lifecycle dependency; по умолчанию пустой lifecycle. EcaScopeLifecycle.Register/Unregister управляют несколькими listeners (attach до начала работы topology); callbacks ConnectScope/DisconnectScope/ConnectRule/DisconnectRule являются частью Core operation, не best-effort notifications. CreateRule ничего не подключает; lifecycle Rule относится только к Register/Unregister membership, со Scope в identity.

EcaScopeLifecycle транзакционно вызывает listeners в порядке регистрации; при ошибке вызывает inverse operations уже завершившихся listeners в обратном порядке. Каждый listener обязан быть атомарным при собственной ошибке. EcaSystemLifecycleCoordinator — один из listeners, а не замена composition root; он аналогично компенсирует выполненные IEcaSystemLifecycleConnector. Compensation продолжает остальные undo после ошибки и возвращает AggregateException с исходной и rollback errors. Невозможно гарантировать возврат произвольных внешних side effects, если сам callback/undo нарушает atomic contract.

EcaSystem имеет один optional LifecycleConnector. EcaSystemConnector только регистрирует/удаляет его в EcaSystemLifecycleConnectorRegistry вместе с exports в существующей транзакции; topology callbacks сам не выполняет. Registry keyed by System ID, не допускает один connector instance у двух Systems, snapshots упорядочены ordinal System ID. EcaSystemsRuntime связывает registries, ScopeRuntime, lifecycle и coordinator. После Connect exports coordinator синхронизирует новый connector с существующей topology; failure откатывает bindings, затем exports. DisconnectSystem сначала снимает bindings, затем exports; failure удаления exports восстанавливает bindings. Systems без connector продолжают работать без dummy implementation.

Late connect использует snapshot authoritative ScopeRuntime + EcaBaseRuleRegistry.GetSnapshot(), без второго списка Scope/Rules: parent-first DFS (roots/siblings ordinal ScopeId), ConnectScope → Rules в registration order → children. Disconnect — обратный child-first порядок через DisconnectScope. При компенсации Scope восстанавливаются также его current Rules из registry snapshot. Shared Rule instance в нескольких Scope получает отдельные callbacks. Local Rules при DisconnectSystem не удаляются.

Scope.Dispose использует две фазы: snapshot subtree в deterministic child-first порядке → prepare DisconnectScope всех узлов без удаления topology → только после полного успеха commit Core removal child-first. Failure prepare восстанавливает предыдущие bindings, исходные Scope instances/Rules/Groups остаются live и IsDisposed=false. Commit не вызывает пользовательские callbacks; resurrection отсутствует. ScopeRuntime.Dispose так же готовит весь forest. EcaSystemsRuntime при ошибке Scope prepare остаётся доступным для retry; после успешного закрытия ScopeRuntime cleanup exports сохраняет прежнее aggregate/idempotent behavior. Начатые Actions не отменяются. Fire остаётся local, immediate/reentrant; topology/System mutation из lifecycle callbacks отклоняется guard, чтобы prepare snapshot оставался целостным. Обычные mutation из Condition/Action вне lifecycle сохраняются.

ScopeId **globally unique среди active Scope одного ScopeRuntime**, не sibling-local. ParentScopeId не входит в Core identity. После Dispose возможно создать новый instance с тем же ID; stale emitter остаётся привязан к старому instance. Full path в public Scope API не добавлен.

Все актуальные Core2/Variables *Registry предоставляют GetSnapshot(): read-only point-in-time membership, exact references существующих объектов, не deep freeze metadata/state. Event/Rule/ExecutionGroup interfaces включают этот API; прежние live collections остаются. StateRegistry выдаёт snapshot строковых IDs, не раскрывая internal resolver records/delegates. VariableRegistry выдаёт facade references; для point-in-time values используются GetState/GetStoreState/GetSubtreeState. Generic Registry base не добавлен.

TimeSystem и VariablesSystem — внешние Systems, планируемые к отдельным пакетам; Core не владеет их state или lifespan. Lifecycle connector управляет согласованными bindings, а не standalone System. Произвольные внешние adapters (включая Time) остаются caller-owned; generic adapter framework, Global Store, queues/cancellation/routing не добавлены.
...
Standalone Variables Core V3 и Variables/Eca реализованы отдельно от Core2. Whole-forest/Store/Subtree snapshots, State exports, Commands и scoped lifecycle binding описаны в локальных Variables docs. Actual state остаётся внешним; product roadmap не входит в общий Core2 ToDo.
...
ConnectSystem/DisconnectSystem используют EcaSystemConnector.Connect/Disconnect и transactional late-sync coordinator, описанный выше, без aliases Attach/Detach. Все scopes видят один live global EventRegistry, но имеют собственные Rule/Execution registries и scoped emitter. Scope, созданный до ConnectSystem, видит подключённые позднее exports. DisconnectSystem не удаляет local Rules: Fire отключённого Event отклоняется registry, повторный Connect того же System/exact exports восстанавливает Fire с прежними Groups/counters.

Dispose временно закрывает public operations во время Scope prepare. Если prepare не прошёл, topology/bindings восстанавливаются и Runtime снова доступен. После успешного закрытия ScopeRuntime удаляется snapshot connected Systems; errors export cleanup накапливаются с прежним AggregateException/idempotent contract. Running Actions не отменяются. Coordinator снимает declared System lifecycle bindings; произвольные внешние adapters/subscriptions Runtime не хранит и не disconnect'ит.

Class/delegate CreateRule и AEcaAction/AEcaCondition реализованы. Context composition/capability container, context-typed helpers и cross-scope routing отложены. Local exports после Connect должны быть стабильны по convention; freeze/snapshot/ownership/consistency и Rule ownership при Disconnect — Roadmap. Следующие общие шаги — Sandbox и Unity authoring; product tasks внешних Systems находятся в их локальной документации.
...
Connect проверяет conflicts до mutation и регистрирует exact instances из system.Events.Events и system.Commands.Commands: Namespace → Events → Commands → States → optional LifecycleConnector → System. Disconnect проверяет canonical identity System, Namespace и всех exports до удаления: optional LifecycleConnector → System → States → Commands → Events → Namespace. Foreign instance с тем же ID не удаляется. Local registries сохраняются; Connector не владеет adapter subscriptions.
...
Systems layer содержит State registrations/resolver, но не владеет actual external SystemState. Queries, enable/disable, serialization, routing и generic adapter framework не реализованы. Optional lifecycle connector управляет конкретными Scope/Rule bindings (см. актуальный раздел выше). Отдельный Time/Eca уже реализован. PR #17 follow-up добавляет focused registry/Connector tests, включая Connect/Disconnect rollback и canonical identity.
