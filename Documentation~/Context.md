...
`TimeTicker.Instance` — единственный public singleton clock source. Public read-only CurrentTime/CurrentUnscaledTime имеют тип double; событие TimeTick передаёт оба значения после обновления. Только ticker читает Unity Time.timeAsDouble / Time.unscaledTimeAsDouble в обычном frame path. Он устанавливает ровно один hook в конец Update, сохраняя остальные PlayerLoop nodes. Повторная установка idempotent. Нет MonoBehaviour/GameObject/DontDestroyOnLoad или scene ownership. Ошибка подписчика логируется и не мешает другим подписчикам. Invocation list кэшируется при изменении подписок; кадр использует стабильный snapshot подписчиков без GetInvocationList каждый frame.
...
`TimeEcaAdapter(TimeSystem, IEcaEventEmitter)` — live bridge, а `adapter.System` — отдельный passive EcaSystem descriptor. Namespace/System Id = time. Connect подписывает шесть standalone aggregate events; повторный Connect даёт InvalidOperationException, Disconnect снимает подписки и допускает повторный вызов. Обязательная последовательность: Connector.Attach(adapter.System) → adapter.Connect(); adapter.Disconnect() → Connector.Detach(adapter.System). Автоматическая проверка Attach в Connect отсутствует: emitter contract не предоставляет inspection, validation происходит при Fire. Ошибка порядка не откатывает уже совершённый Timer transition.

TimeEcaEvents предоставляет actual typed IEcaEvent<TimerEventState> instances, экспортируемые тем же descriptor: time.timer.started/stopped/paused/resumed/completed/destroyed. TimerEventState — immutable snapshot TimerId/Duration/Elapsed/Remaining/Progress/ScaleMode/State, создаваемый adapter до Fire. Поэтому immediate/reentrant ECA handling не меняет уже переданный event payload.

Команды descriptor: time.timer.create (TimerCreateOptions), time.timer.start/stop/pause/resume/destroy (string ID), time.wait (TimeWaitArgs с Duration/ScaleMode). TimeEcaCommandIds даёт discoverable constants. Все команды используют AEcaCommand<IEcaActionContext,A>, без нового context и business result. Только Wait command делает async Task bridge над standalone Awaitable.

Production Core2 composition, общий lifecycle/ownership adapters и общая typed Event discoverability остаются следующей стадией. Core2 API не менялся. State/Queries, Signals, FireEvent/routing, generic external-adapter framework и deferred Time features не реализованы.
