...
{
    public sealed class EcaEventRegistry
    {
        ...
        public bool IsRegistered(IEcaEvent ecaEvent)
        {
            return ecaEvent != null && ecaEvent.Id != null &&
                _events.TryGetValue(ecaEvent.Id, out var registered) && registered.EventStateType == ecaEvent.EventStateType;
        }

        public void RequireRegistered(IEcaEvent ecaEvent)
        {
            if (ecaEvent == null) throw new ArgumentNullException(nameof(ecaEvent));
            if (!IsRegistered(ecaEvent))
                throw new InvalidOperationException($"Event '{ecaEvent.Id}' is unknown or has an incompatible state type.");
        }
    }
}
